...
class LaneEval(object):
  ...
  def bench(pred, gt, y_samples):
    ...
    return ann_in_gt,ann_in_pred,len(gt),len(pred),matched,fp,fn,s / max(min(4.0, len(gt)), 1.), fp / ann_in_pred if ann_in_pred > 0 else 0., fn / max(min(ann_in_gt, 4.) , 1.)
  ...
  def bench_one_submit(pred_file, gt_file):
    ...
    # run_time = pred['run_time']
      ...
      row = [raw_file,len_of_gt,ann_in_gt,len_of_pred,ann_in_pred,matched,fpi,fni]
      rows.append(row)
      accuracy += a
      fp += p
      fn += n
